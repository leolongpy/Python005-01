...
class CrawlThread(threading.Thread):
    ...
    def scheduler(self):
        while not self.queue.empty():
            ...
            url = f'https://pinpai.smzdm.com/1933/wiki/p{page}/'
...
class ParserThread(threading.Thread):
    ...
    def parse_data(self, item):
        ...
        try:
            ...
            books = html.xpath('//*[@id="feed-main"]/div[1]/ul/li')
            for book in books:
                try:
                    title = book.xpath('./div[@class="card-youhui-r"]/a/text()')
                    link = book.xpath('./div[@class="card-youhui-r"]/a/@href')
                    response = {
                        'title': title,
                        'link': link
                    }
